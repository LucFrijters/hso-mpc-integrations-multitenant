...
<#
.SYNOPSIS
    Main orchestrator: loads the tenant configuration and the collection registry, then fans out
...
$partners = @($tenantsConfig | Where-Object { $_.Enabled -eq $true })
...
foreach ($batch in $partnerBatches) {
    ...
    foreach ($partner in $batch) {
        $subInput = @{
            ...
            CollectPartnerInsights      = $partner.CollectPartnerInsights
            CollectPartnerSecurityScore = $partner.CollectPartnerSecurityScore
